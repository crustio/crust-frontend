      ...
      <div class="start-code">
        <div class="start-code-left">
          <div class="start-code-left-top">
            <div class="start-code-left-tab start-pc">
              <div @click="activeTab = 1" :class="{
                'start-code-tab': true,
                'start-code-tab-active': activeTab == 1,
              }">
                {{ $t(`storage.crustApi`) }}
              </div>
              <div @click="activeTab = 2" :class="{
                'start-code-tab': true,
            ...
            <div class="start-code-content">
              <div class="start-code-main" v-if="activeTab == 1" >
                <p>import { <span style="color:#c0392b">ApiPromise, WsProvider</span> } from
                  &#39;@polkadot/api&#39;;<br />
                  import { <span style="color:#c0392b">typesBundleForPolkadot, crustTypes</span> } from
                  &#39;@crustio/type-definitions&#39;;<br />
                ...
                <p><span style="color:#c0392b">&nbsp;const </span>placeStorageOrder = <span style="color:#c0392b">async
                  </span>() =&gt; {</p>

                <p><span style="color:#27ae60">&nbsp;// 1. Connect to chain</span><br />
                  &nbsp;<span style="color:#c0392b">const </span>api = new <span
                    style="color:#2980b9">ApiPromise</span>({<br />
                  &nbsp; &nbsp; &nbsp;provider: new <span
                    style="color:#3498db">WsProvider</span>{{htmlContent}},<br />
                  &nbsp; &nbsp; &nbsp;typesBundle: typesBundleForPolkadot,<br />
                  &nbsp;});</p>

                <p>&nbsp;<span style="color:#27ae60">// 2. Construct place-storage-order tx</span><br />
                  &nbsp;<span style="color:#c0392b">const </span>fileCid = &#39;Qm123&#39;; <span
                    style="color:#27ae60">// IPFS CID</span><br />
                  &nbsp;<span style="color:#c0392b">const </span>fileSize = 2 * 1024 * 1024 * 1024; <span
                    style="color:#27ae60">// Let&#39;s say 2 gb(in byte)</span><br />
                  &nbsp;<span style="color:#c0392b">const </span>tips = 0;<br />
                  &nbsp;<span style="color:#c0392b">const </span>memo = &#39;&#39;;<br />
                  <span style="color:#c0392b">&nbsp;const </span>tx = api.tx.market.<span
                    style="color:#3498db">placeStorageOrder</span>(fileCid, fileSize, tips, memo);
                </p>

                <p>&nbsp; <span style="color:#27ae60">&nbsp; // 3. Send and monitor storage order tx with
                    seeds(account)</span><br />
                  <span style="color:#c0392b">&nbsp;const </span>seeds = &#39;xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx
                  xxx xxx&#39;;<br />
                  <span style="color:#c0392b">&nbsp;const </span>kr = new <span style="color:#3498db">Keyring</span>({
                  type: &#39;sr25519&#39; });<br />
                  <span style="color:#c0392b">&nbsp;const </span>krp = kr.addFromUri(seeds);<br />
                  <span style="color:#c0392b">&nbsp;await </span>tx.<span style="color:#3498db">signAndSend</span>(krp,
                  ({events = [], status}) =&gt; {<br />
                  &nbsp; &nbsp; &nbsp;<span style="color:#c0392b">console</span>.log(`💸 Tx status: ${status.type},
                  nonce: ${tx.nonce}.`);
                </p>

                <p>&nbsp; &nbsp; &nbsp; &nbsp; if (status.isInBlock) {<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; events.<span style="color:#3498db">forEach</span>(({event:
                  {method, section}}) =&gt; {<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;if (method === &#39;ExtrinsicSuccess&#39;) {<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<span
                    style="color:#c0392b">console</span>.log(`✅ &nbsp;Place storage order success!`);<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;}<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; });<br />
                  &nbsp; &nbsp; &nbsp; &nbsp; } else {<br />
              ...
              <div class="start-code-main" v-if="activeTab == 2">
                ...
                <p><span style="color:#c0392b">const </span>pinToCrust = <span style="color:#c0392b">async </span>()
                  =&gt; {</p>
                <p><span style="color:#27ae60">&nbsp; &nbsp;// 1. Construct web3 auth header</span></p>
                <p><span style="color:#27ae60">&nbsp; &nbsp;// Now support: ethereum-series, polkadot-series, solana,
                    elrond, flow, near, ...</span></p>
                <p><span style="color:#27ae60">&nbsp; &nbsp;// Let&#39;s take ethereum as example</span></p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>pair = ethers.Wallet.<span
                    style="color:#2980b9">createRandom</span>();</p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>sig = <span style="color:#c0392b">await
                  </span>pair.<span style="color:#2980b9">signMessage</span>(pair.address);</p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>authHeaderRaw = `eth-${pair.address}:${sig}`;
                </p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>authHeader = Buffer.<span
                    style="color:#2980b9">from</span>(authHeaderRaw).toString(&quot;base64&quot;); <span
                    style="color:#27ae60">// IPFS Web3 Authed Pinning Service Endpoint</span></p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>ipfsW3PinningService =
                  &quot;https://pin.crustcode.com/psa&quot;;</p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>ipfsCid = &quot;Qm123&quot;;</p>
                <p>&nbsp; &nbsp;<span style="color:#c0392b">const </span>{ statusCode } = <span
                    style="color:#c0392b">await </span>got.<span style="color:#3498db">post</span>(ipfsW3PinningService,
                  {</p>
                <p>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; headers: {</p>
                ...
                <p>&nbsp; &nbsp; &nbsp; <span style="color:#c0392b">console</span>.log(&quot;Pin to Crust
                  successfully!&quot;);</p>
                <p>&nbsp; &nbsp;} else {</p>
                <p>&nbsp; &nbsp; &nbsp; <span style="color:#c0392b">console</span>.error(&quot;Pin to Crust
                  failed!&quot;);</p>
                <p>&nbsp; &nbsp;}</p>
                <p>&nbsp;};</p>
              </div>
        ...
        <div class="start-code-right">
          <div class="start-code-right-butto" @click="routeTo('start_button1')">
            <div>
              <img src="../../../assets/img/4-Start using Crust storage interfaces/Dev Guidance .svg" alt="" />
            </div>
            {{ $t(`storage.devButton`) }}
          </div>
          <div class="start-code-right-butto" v-if="activeTab == 2" @click="routeTo('start_button4')">
            <div>
              <img src="../../../assets/img/4-Start using Crust storage interfaces/storage.svg" alt="" />
            </div>
          ...
          <div class="start-code-right-info start-code-right-info1" v-if="activeTab == 1"
            v-html="$t(`storage.crustInfo`)"></div>
          <div class="start-code-right-info start-code-right-info2" v-if="activeTab == 2"
            v-html="$t(`storage.crustChainINfo`)"></div>
        </div>
      </div>
    </div>
  </div>
