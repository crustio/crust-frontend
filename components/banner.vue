    ...
    <div class="hero_area pt-[140px] md:pt-[215px] pb-[60px] md:pb-[125px] bg-cover bg-left-bottom reactive">
        ...
        <div class="container px-3 z-10 relative">
                ...
                <div class="w-full sm:w-10/12 px-3 text-center pb-[100px] md:pb-[140px]">
                    ...
                    <div class="w-11/12 sm:w-10/12 md:w-8/12 flex flex-wrap items-center gap-4 md:gap-8 mx-auto justify-center">
                        <Button class="min-w-[170px] lg:min-w-[210px] btn_one">
                            <a href="https://wiki.crust.network/docs/en/build101" target="_blank" rel="noopener noreferrer">Build on Crust</a>
                        </Button>
                        <Button class="min-w-[170px] lg:min-w-[210px] btn_two">
                            <a href="https://apps.crust.network/?rpc=wss%3A%2F%2Frpc.crust.network#/explorer" target="_blank" rel="noopener noreferrer">Crust Mainnet</a>
                        </Button>
            ...
            <div class="flex flex-wrap -mx-3 justify-center">
                <div class="lg:w-1/4 md:w-1/3 w-full px-3 mb-10 md:mb-7 text-center" v-for="item in bannerInfo" :key="item.title">
                    <h3 class="text-[24px] md:text-[32px] font-bold leading-tight mb-3 text-white">{{ item.title }} <sub>{{ item.sub }}</sub></h3>
...
<script setup>
    let totalStorage = JSON.parse(await $fetch('https://sd.crustcode.com/api/totalStorage'))?.data || 0
    let totalValidNodes = JSON.parse(await $fetch('https://sd.crustcode.com/api/totalValidNodes'))?.data || 0
    let orderCount = JSON.parse(await $fetch('https://sd.crustcode.com/api/orderCount'))?.data || 0
    let filePrice = JSON.parse(await $fetch('https://sd.crustcode.com/api/filePrice'))?.data || 0
